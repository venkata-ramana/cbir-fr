...
class PyFaces:
    def __init__(self,testimg,imgsdir,egfnum,thrsh):
        ...
        self.mindist, self.matchfile=self.facet.findmatchingimage(self.testimg,self.egfnum,self.threshold)
        if self.mindist < 1e-10:
            self.mindist=0
        if not self.matchfile:
            #print "NOMATCH! try higher threshold"
            self.matchfile='NOMATCH! try higher threshold'
        # else:
        #     print "matches :"+self.matchfile+" dist :"+str(self.mindist)
    
    def getMatchFile(self):
        return (self.matchfile, self.mindist)
